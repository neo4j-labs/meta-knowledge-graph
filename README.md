...
| `SessionStart` (`startup\|resume\|clear`), `UserPromptSubmit` | `hooks/inject_project_context.py` | Ranks `:Learning` against the new prompt (hybrid vector + fulltext, RRF) and injects the top hits: project-scoped learnings for the current project, plus durable user-scoped learnings that follow the user across every project. Prompt-driven recall is relevance-gated: vector hits below a cosine-similarity floor (`MKG_INJECT_MIN_SIMILARITY`, default 0.7) are dropped, and when retrieval runs but nothing clears the bar the hook injects nothing at all rather than padding the context from the fulltext or recency fallbacks — those remain only for databases without embeddings and for the no-prompt `SessionStart` path, which stays recency-based by design. Every injected item is linked to the session via `[:INJECTED_IN]`; items already injected earlier in the same session — or first produced during it (`[:FROM_SESSION]`) — are excluded from retrieval, so a conversation never receives the same learning twice, nor has its own freshly-extracted memory echoed back. Marks served learnings as used. At `SessionStart` it additionally injects a "Recent project activity" block — the latest `:Observation` timeline entries by recency (never similarity) as **one-line headlines only**, plus a count of older episodes and a pointer to `episode_fetch` / `episode_search`, so the "previously on this project" recap stays flat and the bodies stay pull-based. |
